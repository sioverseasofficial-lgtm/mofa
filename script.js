    ...
    if (dataParam) {
        ...
        fileInput.addEventListener('change', async function(e) {
            ...
            if (file) {
                ...
                uploadStatus.textContent = 'Uploading high quality image to cloud...';
                ...
                try {
                    ...
                    const response = await fetch("https://api.imgur.com/3/image", {
                        method: "POST",
                        headers: {
                            Authorization: "Client-ID 34b90f75e2fbd03"
                        },
                        body: formData
                    });
                    ...
                    if (data.success) {
                        ...
                        uploadStatus.textContent = 'Upload successful! High quality image secured (Imgur).';
                    ...
                    console.log("Imgur upload failed, trying Cloudinary...");
                    try {
                        // Attempt 2: Cloudinary Demo API (Very reliable CORS fallback)
                        const cloudFormData = new FormData();
                        cloudFormData.append("file", file);
                        cloudFormData.append("upload_preset", "docs_upload_example_us");
                        
                        const cloudResponse = await fetch("https://api.cloudinary.com/v1_1/demo/image/upload", {
                            method: "POST",
                            body: cloudFormData
                        });
                        ...
                        if (cloudData.secure_url) {
                            photoPreview.src = cloudData.secure_url;
                            photoPreview.style.display = 'block';
                            base64Input.value = cloudData.secure_url;
                        ...
                        console.error("All high-quality cloud uploads failed:", cloudError);
                        uploadStatus.textContent = 'Network blocked cloud upload. Trying low-quality offline fallback...';
                        uploadStatus.style.color = '#dc2626';
                        
                        // Fallback to the aggressive compression if all cloud uploads fail
                        compressImage(file, function(dataUrl) {
                            photoPreview.src = dataUrl;
                            photoPreview.style.display = 'block';
                            base64Input.value = dataUrl;
                            uploadStatus.textContent = 'Uploaded using low-quality offline fallback. Image will be blurry.';
